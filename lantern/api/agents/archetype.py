...
logger = logging.getLogger("lantern.archetype")


# -----------------------------------------------------------------------------
# REGEX FAST-PATH
# -----------------------------------------------------------------------------
# ~70% of titles in real cycles are unambiguous: a title containing
# "Technical Program Manager" is `tpm`, full stop. Pre-fix, we paid an
# LLM call (~3-5s on local Ollama) per matched job to confirm what a
# regex can decide in microseconds. The fast-path matches the obvious
# cases here; only ambiguous titles fall through to the LLM.
#
# Patterns are evaluated in declared order — first match wins. Order
# matters: more specific patterns (TPM, AI PM) come BEFORE the broad
# "Product Manager" catch-all so we don't wrongly bucket
# "Technical Program Manager" as `pm`.
#
# A `None` return from this fast-path means "ambiguous, ask the LLM"
# — different from `unclassified`, which is a real verdict.
# -----------------------------------------------------------------------------
_FAST_PATH_PATTERNS: tuple[tuple[re.Pattern[str], str], ...] = (
    # Director / VP / Head-of — title is decisive even when the JD
    # text talks about IC work, because the LinkedIn-grade title IS
    # the role. The match-time trapdoor will drop these for sub-8-yoe
    # candidates anyway, but the bucket is still right.
    (re.compile(r"\b(?:director|vp|vice president|head of|chief|cxo|cto|cpo)\b", re.I), "director"),
    # TPM / Program Manager — explicit "Technical Program Manager",
    # "Program Manager", "TPM" all bucket here. CHECKED BEFORE AI/ML
    # because "Technical Program Manager, AI" is a TPM coordinating
    # AI work — the role shape is coordination, not IC product. If we
    # checked AI first, the bare word "AI" in the title would pull
    # these into ai_pm and mislabel the seat.
    (re.compile(r"\b(?:technical program manager|program manager|tpm)\b", re.I), "tpm"),
    # AI / ML PM — the model-name-y / agent-y vocabulary is unique
    # enough that we can decide on title alone (after we've ruled
    # out TPM above).
    (re.compile(r"\b(?:ai|ml|llm|gen-?ai|machine learning|model|agent|inference|rag|foundation|prompt)\b", re.I), "ai_pm"),
    # Platform / Infrastructure PM — "Platform", "Infrastructure",
    # "Developer Platform", "Internal Tools" patterns.
    (re.compile(r"\b(?:platform|infra|infrastructure|developer platform|internal tools|api|sdk|devtools|dev tools)\b", re.I), "platform_pm"),
    # Growth PM — explicit growth/funnel/activation vocabulary.
    (re.compile(r"\b(?:growth|funnel|activation|monetization|retention|acquisition)\b", re.I), "growth_pm"),
    # Product Operations — "Product Operations", "ProductOps", "PMO".
    (re.compile(r"\b(?:product operations|product ops|productops|pmo)\b", re.I), "ops_pm"),
    # Plain "Product Manager" / "Senior PM" / "Group PM" with no other
    # qualifier — the catch-all for the boring core PM role.
    (re.compile(r"\b(?:product manager|product management)\b", re.I), "pm"),
    # Adjacent but explicitly NOT-PM titles — Designer, Engineer,
    # Researcher, Marketer.
    (re.compile(r"\b(?:designer|engineer|researcher|marketing manager|brand manager|content manager)\b", re.I), "other"),
)


def _classify_by_title_regex(title: str) -> str | None:
    """Return an archetype slug if the title is unambiguous, else None.

    The fast-path catches ~70% of real PM titles without calling the
    LLM. None means "fall through to the LLM" — used for titles that
    are too short, too cryptic, or genuinely ambiguous.
    """
    if not title:
        return None
    t = title.strip()
    if len(t) < 3:
        return None
    for pattern, slug in _FAST_PATH_PATTERNS:
        if pattern.search(t):
            return slug
    return None
...
def classify_archetype(
    ...
    *,
    ...
    model / config_models :
        ...
        (which falls back to the always-resident default model —
        currently qwen3:8b).
    ...
    # Fast-path: regex on the title decides ~70% of real-world cases
    # in microseconds. Only ambiguous titles fall through to the LLM.
    # Pre-fix, we paid one ~3-5s LLM call per matched job (~250 jobs
    # per cycle × 4s ≈ 17 minutes wasted). The fast-path slug is
    # returned with a high confidence (0.85) so the UI treats it
    # the same as an LLM verdict — the regex IS strong evidence
    # for these specific patterns.
    fast = _classify_by_title_regex(title)
    if fast is not None:
        return {
            "archetype": fast,
            "confidence": 0.85,
