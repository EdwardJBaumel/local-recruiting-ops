...
logger = logging.getLogger("lantern.server")
...
class SentinelHandler(BaseHTTPRequestHandler):
    ...
    def _serve_static(self, path: str):
        ...
        if not root.is_dir():
            self._send_json({
                "error": "UI not built",
                "hint": "Run: cd lantern/ui ; npm install ; npm run build",
                "looked_in": str(root),
            }, status=503)
            return
            ...
            try:
                ...
                with _state_lock:
                    ...
                    if "ingest" in new_config and isinstance(new_config["ingest"], dict):
                        ...
                        # lantern/ui/src/components/settings/CompaniesSection.tsx.
...
SUMMARY:"""
            ...
            try:
                ...
                # patches in lantern/ui/src/hooks/useReact.applyReaction
...
def start_server(port=8099, host="127.0.0.1"):
    ...
    try:
        ...
        msg = f"Lantern API could not bind to {host}:{port} ({e})."
        hint = (
            "Another process is probably holding the port. Close any old "
            "Lantern window and try again, or set api_port in config.json "
            "to something else (e.g. 8100)."
        )
        logger.error(msg)
        print(f"[lantern.server] ERROR: {msg}\n[lantern.server] HINT: {hint}", flush=True)
        raise
    logger.info("Lantern API on http://%s:%d", host, port)
    print(f"[lantern.server] listening on http://{host}:{port}", flush=True)
...
def start_server_thread(port=8099, host="127.0.0.1"):
    ...
    try:
        ...
        hint = (
            "Close the other Lantern window (Task Manager > python.exe), "
