...
Signals:
  ...
  f. missing_fields      — description / company / title missing or thin
                           (NOT technologies — JSON ATS APIs never ship it)
...
def _score_missing_fields(payload: dict) -> Optional[tuple]:
    ...
    'technologies' is intentionally NOT checked. The JSON ATS APIs
    (Greenhouse, Lever, Ashby, Workday) don't ship a structured
    technologies field at all — penalising its absence flagged every
    posting from those sources (most of the registry) for a data-source
    quirk that says nothing about whether the role is a ghost. A thin
    DESCRIPTION is the real thinness tell, and that stays.
    """
...
# Per-signal severity-floor cap (Apr 2026 fix).
#
# The previous severity-floor logic was a universal `max_fired * 0.70` —
# meaning ANY signal saturating at 1.0 dragged the raw score up to 0.70,
# which calibrated to exactly 79% display. The result: every job with
# Amazon-style boilerplate ("passionate", "fast-paced" × 6) hit the
# same 79 as a 200-day-old genuine ghost posting. Indistinguishable.
#
# These caps say "this signal, alone, can lift the floor to AT MOST X".
# Hard signals (age_stale, duplicate_title, missing_fields) keep meaningful
# floors; soft / circumstantial signals (buzzword_density, location_vague,
# salary_band_wide) cap lower so they can't single-handedly Suspect-tier
# a fresh, well-described posting. Mixed signals still combine through the
# weighted-average path, which hasn't changed.
_SIGNAL_FLOOR_CAP = {
    "age_stale":          0.70,   # 200d+ posting → 79% display, kept
    "duplicate_title":    0.65,   # near-cert ghost when re-posted same loc
    "missing_fields":     0.55,
    "seniority_conflict": 0.50,
    "apply_url_missing":  0.50,
    "salary_band_wide":   0.40,
    "location_vague":     0.40,
    "overloaded_stack":   0.40,
    "buzzword_density":   0.28,   # corporate boilerplate ≠ ghost evidence
}
...
def score_fake(payload: dict, title_index: Optional[dict] = None,
    ...
    # Severity floor — per-signal capped (Apr 2026 fix).
    #
    # The weighted average alone dilutes a strong single signal — a
    # 72-day-old posting scores age=0.85 but the weighted average is
    # only 0.17 after everything else scores 0. That reads as "mostly
    # fine" even though age alone is damning.
    #
    # We let each signal lift the score to its OWN floor cap (see
    # _SIGNAL_FLOOR_CAP above), then take the max across signals. The
    # caps differentiate hard evidence (age, dup-title) from soft
    # evidence (buzzwords, vague location). Without per-signal caps,
    # buzzword_density=1.0 alone produced the same 79% display as an
    # ancient duplicate — they're not the same, and the UI now reflects
    # that.
    per_signal_floors = [
        s["score"] * _SIGNAL_FLOOR_CAP.get(name, 0.45)
        for name, s in signals.items()
    ]
    max_floor = max(per_signal_floors, default=0.0)
    raw = max(weighted_avg, max_floor)
    ...
    # Missing-posted-date soft cap.
    #
    # If we have NO age signal at all (the source feed didn't include
    # a posted_date and we couldn't parse one), age — the single most
    # reliable ghost-detection signal — is missing. Other signals can
    # still flag, but it's wrong to leap to "Suspect" with the most
    # important piece of evidence absent. We cap the displayed score at
    # the top of the Caution band (0.29 calibrated, just under the 0.30
    # warn threshold) so undated postings default to Clear.
    #
    # The user can always inspect the signals section in the detail
    # card to see what DID fire — capping is a display choice, not a
    # data-loss one.
    age_outcome = results.get("age_stale")
    if age_outcome is None:
        final = min(final, 0.29)
