...
logger = logging.getLogger("lantern.feedback_learner")
    ...
    def _max_cosine(self, job_vec, state: str, min_count: int = MIN_SET_SIZE) -> float | None:
        ...
        try:
            ...
            if hasattr(job_vec, "cpu"):
                # Force CPU: t_cands is always CPU (built from a Python
                # list), but job_vec is the live match embedding and can
                # arrive on CUDA. A CPU @ CUDA matmul raises a device-
                # mismatch error. These are tiny vectors — CPU is fine.
                t_job = job_vec.detach().to(
