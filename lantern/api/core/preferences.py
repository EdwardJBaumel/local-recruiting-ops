...
logger = logging.getLogger("lantern.preferences")
...
# Country-only location strings — when a posting's `location` field is
# JUST the name of a country (no city), the city-pin filter shouldn't
# treat it as "out of region". These are the common variants we see
# across Greenhouse / Lever / Ashby / Workday feeds. The country-level
# allow/block check has already run upstream by the time the location
# filter looks at this, so reaching here means the country is allowed.
# Match is case-insensitive; we lowercase the input before comparing.
_COUNTRY_ONLY_LOCATIONS = frozenset({
    ...
    # Generic continents / regions that show up as a location field.
    "north america", "europe", "emea", "apac", "americas", "latam",
})


def _is_country_only_location(location: str) -> bool:
    """True when `location` is exactly a country / region name with no
    city. Matched case-insensitively against the curated list above
    (callers should lowercase first; we lowercase again as a safety
    net so a stray uppercase string still matches).
    """
    norm = (location or "").strip().lower()
    return norm in _COUNTRY_ONLY_LOCATIONS
...
class LocationFilter:
    ...
    def __init__(self, prefs: dict):
        ...
        # NOTE: the geographic pin-on-a-map filter was removed in Apr
        # 2026 along with its UI (`PinMap.tsx`). The two preference
        # keys it consumed (`location_pin_areas`, `location_pin_radius_km`)
        # are now ignored if present in older config.json files —
        # we don't raise on them so existing configs keep loading.
    ...
    def evaluate(self, payload: dict) -> tuple[bool, str]:
        ...
        in soft mode we never drop on the substring rules."""
        if self.mode == "soft":
            return True, ""
        return self._eval(payload)
    ...
    def _eval(self, payload: dict) -> tuple[bool, str]:
        ...
        # 3. Inclusion list — substring allowlist. A job passes if it
        # substring-matches any allowed_locations entry. Empty = no
        # filter. Remote bypasses entirely (location-agnostic).
        # Country-only locations ("United States", "USA", "Canada"...)
        # bypass because the country filter ran upstream and would
        # have dropped them if the country wasn't allowed; surviving
        # to here means it is and the missing-city is recruiter
        # laziness, not a real signal to reject on.
        if mode != "remote" and self.allowed:
            if _is_country_only_location(location):
                return True, ""

            if not location:
                # No location text to test against — pass.
                text_passes = True
            else:
                text_passes = any(a in location for a in self.allowed)

            if not text_passes:
                return False, "location: no allowed_locations match"
...
def title_has_blocked_keyword(title: str, blocked_keywords) -> str | None:
    """Return the first blocked keyword found in `title` as a WHOLE WORD,
    or None. Case-insensitive, word-boundary matched.

    Shared by TitleScorer (scoring-time penalty) and
    agents.ingest._matches (scrape-time skip) so the two stay in
    lockstep — the set of titles ingest drops is exactly the set the
    scorer would have penalised. Word-boundary (not substring) matching
    keeps "engineer" from hitting "engineering" and "counsel" from
    hitting "counselor".
    """
...
class TitleScorer:
    """Soft weight. Pushes the match score UP for titles the user
    explicitly asked for, and DOWN hard for wrong-discipline titles.

    The embedding model treats a title and its JD as one equal word-bag,
    so cross-domain cosine bunches nearly every PM-ish posting into
    0.5-0.65 — a title the user actually wants barely separates from a
    tangential role that happens to share JD vocabulary. This scorer is
    the thumb on the scale that fixes that.

    Config keys:
      - role_keywords (list[str]): the user's target titles. A
        whole-word hit applies a flat +title_weight boost.
      - title_weight (float, default 0.20): the positive boost. Flat —
        any whole-word role-keyword hit earns the full bump (the old
        len-scaled version punished short-but-exact titles like "TPM").
      - blocked_title_keywords (list[str]): wrong-discipline markers
        (engineer, designer, counsel, ...). A whole-word hit applies a
        hard -title_penalty and short-circuits — these never earn a
        boost. Shared with the ingest scrape-time skip.
      - title_penalty (float, default 0.60): the blocked-title penalty.
        Large by design: a wrong-discipline title should drop out of the
        match tier regardless of how well its JD reads.
    """
    ...
    def __init__(self, prefs: dict):
        ...
        if isinstance(raw, str):
            raw = [raw]
        # Lowercase + dedupe + sort longest-first so the reason string
        # reports the most specific matching keyword when several hit.
        seen: set[str] = set()
        kws: list[str] = []
        for k in raw:
            s = str(k).strip().lower()
        ...
        self.keywords = kws
        braw = prefs.get("blocked_title_keywords") or []
        if isinstance(braw, str):
            braw = [braw]
        self.blocked_keywords = [
            s for s in (str(b).strip().lower() for b in braw) if s
        ]
        try:
            self.weight = float(prefs.get("title_weight", 0.20) or 0)
        except (TypeError, ValueError):
            self.weight = 0.20
        try:
    ...
    def adjust(self, base_score: float, payload: dict) -> tuple[float, float, str]:
        ...
        # matter how the rest of the title reads.
        if self.penalty > 0 and self.blocked_keywords:
            hit = title_has_blocked_keyword(title, self.blocked_keywords)
            if hit:
                delta = -self.penalty
                return _clamp(base_score + delta), delta, (
                    f"title contains blocked keyword '{hit}' ({delta:.3f})"
                )
