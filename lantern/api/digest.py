...
logger = logging.getLogger("lantern.digest")
...
Start with the exact line "Lantern Weekly Digest -- {today}" (no alteration - use the date exactly as provided). No markdown formatting. Do not invent a date.
"""
...
class DigestGenerator:
    ...
    def _force_correct_header(text: str, today_str: str) -> str:
        ...
        # so digests saved before the Lantern rename still parse correctly.
        header_pat = re.compile(r"(?i)^\s*(?:sentinel|lantern)\s+weekly\s+digest\b.*$")
        ...
        while lines and (header_pat.match(lines[0]) or date_pat.match(lines[0]) or not lines[0].strip()):
            lines.pop(0)
        return f"Lantern Weekly Digest -- {today_str}\n\n" + "\n".join(lines).strip() + "\n"

    def _fallback_digest(self, stats, companies, gaps):
        return (
            f"Lantern Weekly Digest -- {datetime.now().strftime('%B %d, %Y')}\n\n"
    ...
    def send_email(self, text: str):
        ...
        try:
            ...
            msg["Subject"] = f"Lantern Digest -- {datetime.now().strftime('%b %d, %Y')}"
