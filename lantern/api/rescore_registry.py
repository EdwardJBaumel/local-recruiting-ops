...
Run from lantern/api (same cwd as main.py):
...
logger = logging.getLogger("lantern.rescore_registry")
CHUNK_SIZE = 32
...
def _load_config() -> dict:
    ...
    if not path.is_file():
        raise SystemExit(f"config.json not found at {path} — run from lantern/api")
