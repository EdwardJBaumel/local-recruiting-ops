...
The pipeline has five distinct LLM tasks (parse, match, analyse, chat,
digest). Historically each had its own model which meant up to four
...
Profiles (ordered cheap → heavy):
  ...
  b. `compact` — two models: a fast 7-8B for parse/QA, a 12-14B for
     match/analyse/chat/digest. Two resident at most; one swap between
...
`chat.model` to something different.
...
PROFILES: dict[str, LLMProfile] = {
    ...
    "compact": LLMProfile(
        ...
        notes="Two models: fast parse + strong reasoning. Balances VRAM swap cost.",
    ),
    "full": LLMProfile(
        name="full",
        parse="qwen3:14b",
        match="qwen3:14b",
        analyze="phi4-reasoning:14b",
        chat="qwen3:8b",
        digest="gemma3:12b",
        cover_letter="gemma3:12b",
        approx_unique_models=4,
        notes="Per-stage specialisation. Best quality, most swap cost.",
    ),
}
    ...
    return [
        {
            ...
            "chat": p.chat, "digest": p.digest, "cover_letter": p.cover_letter,
