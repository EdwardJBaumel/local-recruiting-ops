#!/usr/bin/env python3
"""
SENTINEL - Multi-Agent Job Matching Pipeline
Run: python main.py
Stop: Ctrl+C

Requires:
  - Ollama running locally (ollama serve) with gemma4:26b pulled
  - pip install requests beautifulsoup4
  - pip install sentence-transformers  (optional, for embedding-based matching)
"""
...
def check_ollama(model: str):
    ...
    try:
        ...
        _fatal(
            ...
            "Start Ollama first: open a terminal and run 'ollama serve', then click SENTINEL again.",
        )
...
def main():
    ...
    # Open the browser ASAP. SENTINEL_NO_BROWSER=1 lets the launcher
    # open :3000 (the Vite dev server) instead during development.
    if os.environ.get("SENTINEL_NO_BROWSER") != "1":
        def _open():
    ...
    def _bg_preflight_prewarm():
        try:
            ...
            configured_models = list({
                (config.get("parse", {}) or {}).get("model") or "gemma4:e4b",
                (config.get("match", {}) or {}).get("model") or "qwen3:8b",
                (config.get("analyze", {}) or {}).get("model") or "qwen3:8b",
                (config.get("chat", {}) or {}).get("model") or "qwen3:8b",
            })
