# lro/ui — Vite + React + TypeScript frontend

The dashboard source. Normal launches serve the built UI from the Python backend on **port 8099** so the user sees one local app URL. Set `LRO_DEV_UI=1` when you specifically want Vite on **port 3000** for UI development; Vite proxies `/api/*` to the backend on port 8099.
