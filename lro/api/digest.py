...
logger = logging.getLogger("lro.digest")
...
Start with the exact line "Local Recruiting Ops Weekly Digest -- {today}" (no alteration - use the date exactly as provided). No markdown formatting. Do not invent a date.
"""
...
class DigestGenerator:
    ...
    def _force_correct_header(text: str, today_str: str) -> str:
        ...
        # The header alternation also matches the pre-rename "SENTINEL" header
        # so digests saved before the Local Recruiting Ops rename still parse correctly.
        header_pat = re.compile(r"(?i)^\s*(?:sentinel|lro)\s+weekly\s+digest\b.*$")
        ...
        while lines and (header_pat.match(lines[0]) or date_pat.match(lines[0]) or not lines[0].strip()):
            lines.pop(0)
        return f"Local Recruiting Ops Weekly Digest -- {today_str}\n\n" + "\n".join(lines).strip() + "\n"

    def _fallback_digest(self, stats, companies, gaps):
        return (
            f"Local Recruiting Ops Weekly Digest -- {datetime.now().strftime('%B %d, %Y')}\n\n"
    ...
    def send_email(self, text: str):
        ...
        try:
            ...
            msg["Subject"] = f"Local Recruiting Ops Digest -- {datetime.now().strftime('%b %d, %Y')}"
