...
logger = logging.getLogger("lro.orchestrator")
...
def _resolve_manual_mode(config: dict, env=None) -> bool:
    ...
    Resolution order:
      a. env LRO_MANUAL_MODE ("1"/"true"/"yes"/"on" → manual);
    ...
    if env is None:
        env = os.environ
    raw = (env.get("LRO_MANUAL_MODE") or env.get("SENTINEL_MANUAL_MODE") or "").strip().lower()
...
class Orchestrator:
    ...
    def run(self):
        logger.info("LRO ORCHESTRATOR STARTING")
        ...
        if manual:
            ...
            try:
                ...
                self._save_seen()
                logger.info("LRO STOPPED (manual mode) after %d cycles.",
                            self.cycle_count)
            return
        ...
        try:
            ...
            self._save_seen()
            logger.info("LRO STOPPED after %d cycles.", self.cycle_count)
