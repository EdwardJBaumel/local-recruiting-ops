...
def bundle_dir() -> Path:
    ...
    # core/app_paths.py -> core -> lro/api/
...
def runtime_dir() -> Path:
    ...
    if is_frozen():
        # LRO_HOME is the canonical name; SENTINEL_HOME stays as a
        # legacy alias so v1 users with that env var don't have to
        # change anything when upgrading.
        override = os.environ.get("LRO_HOME") or os.environ.get("SENTINEL_HOME")
        if override:
...
def static_dir() -> Path:
    """Location of the built React UI (lro/ui/dist)."""
    # When frozen, the build script adds 'ui' under bundle_dir(). In
    # local single-port mode, start.ps1 builds lro/ui/dist and the
