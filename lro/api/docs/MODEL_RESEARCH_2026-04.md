...
The dashboard chat UI was removed; this section is preserved only for
historical context. There's no longer a `core/chat.py`, no `chat`
config block, and no `task="chat"` callers in the codebase. Anything
that previously routed via the chat task now uses the always-resident
