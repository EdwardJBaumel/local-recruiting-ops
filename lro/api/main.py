#!/usr/bin/env python3
"""
Local Recruiting Ops - Multi-Agent Job Matching Pipeline
Run: python main.py
Stop: Ctrl+C
...
def setup_logging(config: dict):
    ...
    log_file = log_config.get("file", "logs/lro.log")
...
def check_ollama(model: str):
    ...
    try:
        ...
        _fatal(
            ...
            "Start Ollama first: open a terminal and run 'ollama serve', then re-launch Local Recruiting Ops.",
        )
...
def main():
    ...
    try:
        ...
        logging.getLogger("lro.startup").warning(
    ...
    print(f"Local Recruiting Ops dashboard: {dashboard_url}")

    # Open the browser ASAP. LRO_NO_BROWSER=1 lets the launcher
    ...
    if (os.environ.get("LRO_NO_BROWSER") or os.environ.get("SENTINEL_NO_BROWSER")) != "1":
        def _open():
